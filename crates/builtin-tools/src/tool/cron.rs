...
use benshu_scheduler::{JobPayload, JobSchedule, Scheduler};
...
impl Tool for CronTool {
    ...
    async fn call(&self, arguments: &str) -> anyhow::Result<String> {
        let args: CronArgs = serde_json::from_str(arguments).map_err(|e| {
            anyhow::Error::from(Error::tool_execution(
                "cron",
                format!("Invalid arguments: {}", e),
            ))
        })?;

        let scheduler = self.scheduler.upgrade().ok_or_else(|| {
            anyhow::Error::from(Error::tool_execution("cron", "Scheduler not available"))
        })?;

        match args.action.as_str() {
            "schedule" => {
                let schedule_json = args.schedule.ok_or_else(|| {
                    anyhow::Error::from(Error::tool_execution(
                        "cron",
                        "schedule required for schedule action",
                    ))
                })?;
                let schedule: JobSchedule = serde_json::from_value(schedule_json).map_err(|e| {
                    anyhow::Error::from(Error::tool_execution(
                        "cron",
                ...
                // Default scheduled turns to the prime agent unless the caller
                // provides a more specific routing layer above this tool.
                let id = scheduler
                    .add_job(
                        args.name,
                        schedule,
            ...
            "cancel" => {
                let id_str = args.id.ok_or_else(|| {
                    anyhow::Error::from(Error::tool_execution(
                        "cron",
                        "id required for cancel action",
                    ))
                })?;
            ...
            _ => Err(anyhow::Error::from(Error::tool_execution(
                "cron",
