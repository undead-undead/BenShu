use crate::agent::memory::{Memory, MemoryManager};
...
impl NamespacedMemory {
    ...
    pub async fn store(
        ...
        let serialized = serde_json::to_string(&entry).map_err(|e| {
            crate::error::Error::Internal(format!("Failed to serialize entry: {}", e))
        })?;
    ...
    /// Batch store multiple values efficiently
    pub async fn batch_store(
        &self,
        namespace: &str,
        entries: Vec<(&str, String, Option<Duration>, Option<String>)>,
    ) -> Result<()> {
        for (key, value, ttl, author) in entries {
            self.store(namespace, key, &value, ttl, author).await?;
        }
        Ok(())
    }

    /// Type-safe store for any serializable type
    pub async fn store_typed<T: Serialize>(
        ...
        self.store(namespace, key, &value_str, ttl, author).await
    }

    /// Type-safe read for any deserializable type
    pub async fn read_typed<T: for<'de> Deserialize<'de>>(
        &self,
        namespace: &str,
        key: &str,
    ) -> Result<Option<T>> {
        match self.read(namespace, key).await? {
            Some(s) => serde_json::from_str(&s).map(Some).map_err(|e| {
                crate::error::Error::Internal(format!("Failed to deserialize typed value: {}", e))
            }),
            None => Ok(None),
        }
    }
    ...
    pub async fn read(&self, namespace: &str, key: &str) -> Result<Option<String>> {
        ...
        let entry: MemoryEntry = serde_json::from_str(content).map_err(|e| {
            crate::error::Error::Internal(format!("Failed to deserialize entry: {}", e))
        })?;
    ...
    pub async fn delete(&self, namespace: &str, key: &str) -> Result<()> {
        ...
        self.store(namespace, key, "", Some(Duration::from_secs(0)), None)
            .await
    }
    ...
    /// Active cleanup of expired data in a namespace
    pub async fn clean_expired(&self, namespace: &str) -> Result<usize> {
        let prefix = format!("{}::", namespace);
        let results = self.memory.search("system", None, &prefix, 1000).await?;
        let mut cleaned = 0;

        for res in results {
            if let Ok(entry) = serde_json::from_str::<MemoryEntry>(&res.content) {
                if entry.is_expired() {
                    let key = res.title.strip_prefix(&prefix).unwrap_or(&res.title);
                    self.delete(namespace, key).await?;
                    cleaned += 1;
                }
            }
    ...
    /// List all namespaces present in the memory
    pub async fn list_namespaces(&self) -> Result<Vec<String>> {
        let results = self.memory.search("system", None, "::", 1000).await?;
        let mut namespaces = std::collections::HashSet::new();
        for res in results {
            if let Some(ns) = res.title.split("::").next() {
                if !ns.is_empty() {
    ...
    /// Start a background worker to periodically clean up expired data
    pub async fn start_cleanup_worker(self: Arc<Self>, interval: Duration) {
            ...
            loop {
                ...
                if let Ok(namespaces) = self.list_namespaces().await {
                    for ns in namespaces {
                        let _ = self.clean_expired(&ns).await;
                    }
                }
            }
        });
