...
fn normalize_artifact_policy_value(value: Value) -> Option<Value> {
    let mut policy = if value.is_null() {
        ...
        || value.get("terms").is_some()
        || value.get("tool_config").is_some()
    {
        value
    } else {
...
fn policy_value_matches_task(value: Option<&Value>, original: &str, lowered: &str) -> bool {
    match value {
...
fn policy_text_matches_task(text: &str, original: &str, lowered: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn artifact_policy_yaml_accepts_tool_config_only_documents() {
        let policy = parse_artifact_policy_yaml(
            "tool_config:\n  novel_studio:\n    target_units: 4000\n    chapter_unit_target: 4000\n    export_format: txt\n",
        )
        .expect("parse")
        .expect("policy");

        assert_eq!(
            policy["tool_config"]["novel_studio"]["chapter_unit_target"],
